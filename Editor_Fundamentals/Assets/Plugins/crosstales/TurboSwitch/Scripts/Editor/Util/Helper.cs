...
namespace Crosstales.TPS.Util
{
   ...
   public abstract class Helper : Crosstales.Common.EditorUtil.BaseEditorHelper
   {
      ...
      private static int assetDBVersion;
      ...
      public static Texture2D Icon_Delete_Big => loadImage(ref icon_delete_big, "icon_delete_big.png");
      ...
      public static Texture2D Icon_Cachefull => loadImage(ref icon_cachefull, "icon_cachefull.png");

      public static Texture2D Icon_Cacheempty => loadImage(ref icon_cacheempty, "icon_cacheempty.png");
...
#else
      public static bool hasActiveArchitecturePlatforms => Config.PLATFORM_WINDOWS || Config.PLATFORM_LINUX;
#endif
      ...
      public static bool hasActiveTexturePlatforms => Config.PLATFORM_ANDROID;

      /// <summary>Checks if a cache for the project exists.</summary>
      /// <returns>True if a cache for the project exists</returns>
      public static bool hasCache => System.IO.Directory.Exists(Config.PATH_CACHE);
            ...
            else
            {
               if (System.IO.Directory.Exists(Config.PATH_CACHE))
               {
                  ...
                  if (isV2enabled)
                  {
                     ...
                     System.Threading.Thread worker = isWindowsEditor ? new System.Threading.Thread(() => scanWindows(path, key)) : new System.Threading.Thread(() => scanUnix(path, key));
                     worker.Start();
                  }
                  else
                  {
                     System.Threading.Thread worker = isWindowsEditor ? new System.Threading.Thread(() => scanWindows(Config.PATH_CACHE, key)) : new System.Threading.Thread(() => scanUnix(Config.PATH_CACHE, key));
                     worker.Start();
                  }
               }
               else
      ...
      /// <summary>Returns true if there is a platform enabled.</summary>
      ...
      /// <summary>Returns true if the cache is being deleted.</summary>
      /// <returns>True if the cache is being deleted.</returns>
      public static bool isDeleting { get; private set; }

      /// <summary>Returns true if V2 of the AssetDatabase is enabled.</summary>
      /// <returns>True if V2 of the AssetDatabase is enabled.</returns>
      public static bool isV2enabled
      {
         get
         {
            if (assetDBVersion == 0)
            {
               ...
               assetDBVersion = isV2 ? 2 : 1;

               /*
               if (EditorPrefs.HasKey("AssetPipelineVersionForNewProjects"))
               {
                   assetDBVersion = EditorPrefs.GetInt("AssetPipelineVersionForNewProjects");
               }
               */
#else
               assetDBVersion = 1;
      ...
      public static bool SwitchPlatform(BuildTarget target, MobileTextureSubtarget subTarget = MobileTextureSubtarget.Generic)
      {
         ...
         if (saveScenes())
         {
            ...
            if (!string.IsNullOrEmpty(Config.EXECUTE_METHOD_PRE_SWITCH))
               InvokeMethod(Config.EXECUTE_METHOD_PRE_SWITCH.Substring(0, Config.EXECUTE_METHOD_PRE_SWITCH.CTLastIndexOf(".")), Config.EXECUTE_METHOD_PRE_SWITCH.Substring(Config.EXECUTE_METHOD_PRE_SWITCH.CTLastIndexOf(".") + 1));
         ...
         {
            Debug.LogWarning("User canceled the switch.");
         }
         ...
         if (saveScenes())
         {
            ...
            if (EditorUserBuildSettings.SwitchActiveBuildTarget(group, target))
            {
               ...
               if (!string.IsNullOrEmpty(Config.EXECUTE_METHOD))
                  InvokeMethod(Config.EXECUTE_METHOD.Substring(0, Config.EXECUTE_METHOD.CTLastIndexOf(".")), Config.EXECUTE_METHOD.Substring(Config.EXECUTE_METHOD.CTLastIndexOf(".") + 1));
...
*/
      ...
      */
      ...
      {
         return Crosstales.Common.Util.FileHelper.ValidatePath(Config.PATH_CACHE + target + getExtension(target, subTarget));
      }
            ...
            try
            {
               ...
               if (Config.COPY_LIBRARY)
               {
                  ...
                  if (!isV2enabled)
                  {
                     ...
                     sb.Append("/MIR /W:2 /R:3 /MT /NFL /NDL /NJH /NJS /nc /ns /np > NUL");
               ...
               if (Config.COPY_ASSETS)
               {
                  ...
                  if (!isV2enabled)
                  {
                     ...
                     sb.Append(savePathExtension);
                     sb.Append("\\Assets\" ");
                     sb.Append("/MIR /W:2 /R:3 /MT /NFL /NDL /NJH /NJS /nc /ns /np *.meta > NUL");
               ...
               if (Config.COPY_ASSETS)
               {
                  ...
                  if (!isV2enabled)
                  {
                     ...
                     sb.Append(Constants.APPLICATION_PATH);
                     sb.Append("Assets\" ");
                     sb.Append("/E /W:2 /R:3 /MT /NFL /NDL /NJH /NJS /nc /ns /np *.meta > NUL");
      ...
      private static string generateWindowsScript(BuildTarget target, string savePathExtension, string loadPathExtension)
      {
         ...
         sb.AppendLine("echo #  Copyright 2016-2022 by www.crosstales.com                                 #");
         sb.AppendLine("echo #                                                                            #");
         ...
         {
            sb.Append(" -executeMethod Crosstales.TPS.Switcher.DefaultMethodAfterSwitch");
         }
