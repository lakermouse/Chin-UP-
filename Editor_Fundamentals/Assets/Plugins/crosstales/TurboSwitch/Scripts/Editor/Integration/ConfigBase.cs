...
namespace Crosstales.TPS.EditorIntegration
{
   ...
   public abstract class ConfigBase : EditorWindow
   {
      ...
      private static BuildTarget targetWindows => Config.ARCH_WINDOWS == 0 ? BuildTarget.StandaloneWindows : BuildTarget.StandaloneWindows64;

      private static BuildTarget targetMac => BuildTarget.StandaloneOSX;
      ...
      protected void showConfiguration()
      {
...
#else
            Config.SHOW_COLUMN_ARCHITECTURE = EditorGUILayout.Toggle(new GUIContent("Column: Arch", $"Enable or disable the column 'Arch' in the 'Switch'-tab (default: {Constants.DEFAULT_SHOW_COLUMN_ARCHITECTURE}, 'Windows' or 'Linux' only.)."), Config.SHOW_COLUMN_ARCHITECTURE);
#endif
            Config.SHOW_COLUMN_TEXTURE = EditorGUILayout.Toggle(new GUIContent("Column: Texture", $"Enable or disable the column 'Texture' in the 'Switch'-tab (default: {Constants.DEFAULT_SHOW_COLUMN_TEXTURE}, 'Android' only)."), Config.SHOW_COLUMN_TEXTURE);
         ...
         {
            Crosstales.Common.Util.FileHelper.ShowFile(Config.PATH_CACHE);
         }
      ...
      protected void v2Banner()
      {
         if (Helper.isV2enabled)
         {
            EditorGUILayout.HelpBox("AssetDatabase V2 activated!" + System.Environment.NewLine + "To have the full speed advantage of this asset, change the project to AssetDatabase V1 if possible.", MessageType.Info);
/*
            GUILayout.BeginHorizontal();
            {
      ...
      protected void tpbBanner()
      {
         ...
         if (Constants.SHOW_TPB_BANNER)
         {
            GUILayout.BeginHorizontal();
            {
               EditorGUILayout.HelpBox("'Turbo Builder PRO' is not installed!" + System.Environment.NewLine + "For multiple and faster builds, please install or get it from the Unity AssetStore.", MessageType.Info);
