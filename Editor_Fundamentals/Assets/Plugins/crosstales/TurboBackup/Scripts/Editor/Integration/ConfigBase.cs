   ...
   public abstract class ConfigBase : EditorWindow
   {
            ...
            if (!EditorApplication.isCompiling && !EditorApplication.isUpdating)
            {
               if (!BAR.isBusy)
               {
                  ...
                  if (Helper.isBackupEnabled)
                  {
                     ...
                     GUILayout.Label($"Last Backup:\t{(Helper.hasBackup ? $"{Config.BACKUP_DATE} (#{Config.BACKUP_COUNT})" : "never")}");

                     //))GUILayout.Label($"Automatic Backup:\t{(AutoBackup.BackupInterval > 0 ? $"{Config.AUTO_BACKUP_DATE.ToString()} (in {(Config.AUTO_BACKUP_DATE - System.DateTime.Now).Minutes}min)" : "disabled")}");
      ...
      protected void showConfiguration()
      {
         scrollPosConfig = EditorGUILayout.BeginScrollView(scrollPosConfig, false, false);
         {
            ...
            AutoBackup.BackupInterval = EditorGUILayout.IntSlider(new GUIContent("Auto Backup Interval", "Automatic backup after the given minutes  (default: 0, 0 = disabled)"), AutoBackup.BackupInterval, 0, 300);
      ...
      protected void showAbout()
      {
         ...
         GUILayout.Label("© 2018-2022 by " + Constants.ASSET_AUTHOR);
