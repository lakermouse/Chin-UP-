   ...
   public static class Builder
   {
      ...
      private static bool build(bool quit, string path, string name, string[] targets, params string[] scenes)
      {
         ...
         foreach (string target in targets)
         {
            ...
            {
               buildPlayer(bt, _filename, _path, scenes); //TODO quit??
            }
            else
            {
               btList.Add(target);
      ...
      private static bool buildPlayer(BuildTarget target, string name, string path, params string[] scenes) //, bool quit = false)
      {
         ...
         if (Helper.hasActiveScenes || scenes?.Length > 0)
         {
            ...
            string buildPath = FileHelper.ValidatePath(path + (Config.ADD_NAME_TO_PATH ? $"{name}_" : string.Empty) + (Config.ADD_VERSION_TO_PATH ? $"{version}_" : string.Empty) + modifier + (Config.ADD_DATE_TO_PATH ? $"_{System.DateTime.Now.ToString(Config.DATE_FORMAT)}" : string.Empty));
