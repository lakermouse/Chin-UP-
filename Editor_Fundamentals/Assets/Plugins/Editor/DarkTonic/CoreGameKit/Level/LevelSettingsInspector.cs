...
namespace DarkTonic.CoreGameKit.EditorScripts
{
    ...
    public class LevelSettingsInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (DTInspectorUtility.IsInPrefabMode(_settings.gameObject))
            {
            ...
            var hasLevelSettings = LevelSettings.Instance != null;

            if (DTInspectorUtility.IsPrefabInProjectView(_settings.gameObject) || !hasLevelSettings)
            {
                if (!hasLevelSettings)
                {
                    ...
                    GUILayout.Space(6);
                    if (GUILayout.Button("Create LevelWaveSettings Game Object", EditorStyles.toolbarButton,
                        GUILayout.Width(240)))
                    {
                ...
                {
                    DTInspectorUtility.ShowColorWarningBox("You are in Project View and cannot edit this.");
                }
            ...
            if (!_settings._isDarkTonicExampleScene && DTInspectorUtility.IsLinkedToDarkTonicPrefabFolder(_settings))
            {
        ...
        if (_maInScene) {
            ...
            var playerStatsHolder = CoreMonoHelper.GetChildTransform(_settings.transform, LevelSettings.WorldVariablesContainerTransName);
...
#if MASTERAUDIO_ENABLED
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Initialized Sound", _settings, ref _isDirty, ref _settings.initializedSound);
#endif
        ...
        if (_settings.masterAudioSoundsExpanded) {
            DTInspectorUtility.BeginGroupedControls();
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Win Sound", _settings, ref _isDirty, ref _settings.winSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Lose Sound", _settings, ref _isDirty, ref _settings.loseSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Level Started Sound", _settings, ref _isDirty, ref _settings.levelStartSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Level Ended Sound", _settings, ref _isDirty, ref _settings.levelEndSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Wave Skipped Sound", _settings, ref _isDirty, ref _settings.waveSkippedSound);
            DTInspectorUtility.EndGroupedControls();
        }
        ...
        DTInspectorUtility.EndGroupHeader();
            ...
            if (!Application.isPlaying)
            {
                ...
                if (GUILayout.Button(new GUIContent("Refresh All", "Refresh all wave visualizations"), EditorStyles.toolbarButton, GUILayout.Width(70)))
                {
                    ...
                    for (var t = 0; t < trigSpawners.Length; t++)
                    {
                        ...
                        var waveCollection = trigSpawner.AllWaves;
                        foreach (var key in waveCollection.Keys)
                        {
                            allWaves.AddRange(waveCollection[key]);
                        }
                ...
                {
                    _settings.gameObject.DestroyChildrenIfAnyInPrefab();

                    var visuals = FindObjectsOfType(typeof(VisualizationMarker));
                    var totalItems = visuals.Length;

                    var deadMarkerObjects = new List<GameObject>();
                    var i = 0;

                    while (i < totalItems)
                    {
                        var deadObject = ((VisualizationMarker)visuals[i]).gameObject;
                        i++;

                        // can't delete nested prefab
                        if (PrefabUtility.IsPartOfPrefabInstance(deadObject))
                        {
                            continue;
                        }

                        deadMarkerObjects.Add(deadObject);
                    }

                    for (var j = 0; j < deadMarkerObjects.Count; j++)
                    {
                        DestroyImmediate(deadMarkerObjects[j]);
                    }
                }
            ...
            if (Application.isPlaying && PoolBoss.IsServer)
            {
                ...
                if (!LevelSettings.WavesArePaused && hasNextWave && !LevelSettings.IsGameOver)
                {
                    ...
                    if (GUILayout.Button("Next Wave", EditorStyles.miniButton, GUILayout.Width(80)))
                    {
            ...
            var poolingHolder = CoreMonoHelper.GetChildTransform(_settings.transform, LevelSettings.KillerPoolingContainerTransName);
            ...
            if (_settings.createPrefabPoolsExpanded)
            {
                ...
                if (!Application.isPlaying)
                {
                    ...
                    if (_settings.newPrefabPoolExpanded)
                    {
                        ...
                        GUILayout.Space(2);
                        GUI.contentColor = DTInspectorUtility.AddButtonColor;
                        if (GUILayout.Button("Create Prefab Pool", EditorStyles.toolbarButton, GUILayout.MaxWidth(120)))
                        {
                            CreatePrefabPool();
                        }
                        GUI.contentColor = Color.white;
                ...
                if (_settings.useWaves)
                {
                    ...
                    if (_settings.showCustomWaveClasses)
                    {
                        ...
                        for (var i = 0; i < _settings.customWaveClasses.Count; i++)
                        {
                            ...
                            if (GUILayout.Button(new GUIContent("Delete", "Click to delete Custom Wave Class"), EditorStyles.miniButton, GUILayout.MaxWidth(51)))
                            {
                                classToDelete = i;
                            }
                            ...
                            for (var w = 0; w < filteredWaves.Count; w++)
                            {
                                ...
                                if (waveSetting.isExpanded)
                                {
                                    ...
                                    if (waveSetting.waveType == LevelSettings.WaveType.Timed)
                                    {
                                        ...
                                        var newEnd = EditorGUILayout.Toggle("End When All Killed", waveSetting.endEarlyIfAllDestroyed);
                                        if (newEnd != waveSetting.endEarlyIfAllDestroyed)
                                        {
                                            UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle End Early When All Killed");
                                    ...
                                    foreach (var spawner in spawnersUsed)
                                    {
                                        ...
                                        if (!Application.isPlaying)
                                        {
                                            ...
                                            if (GUILayout.Button(new GUIContent("Delete", "Click to Demove Spawner Wave"),
                                                EditorStyles.miniButton, GUILayout.Width(51)))
                                            {
                                        ...
                                        if (waveSetting.useTriggeredSpawners)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                for (var t = 0; t < waveSetting.trigSpawnerWavesToAwait.Count; t++)
                                                {
                                                    ...
                                                    var oldBG = GUI.backgroundColor;
                                                    GUI.backgroundColor = DTInspectorUtility.DeleteButtonColor;
                                                    if (GUILayout.Button(new GUIContent("Delete", "Click to delete Triggered Spawner"), EditorStyles.miniButton, GUILayout.MaxWidth(51)))
                                                    {
                                            ...
                                            switch (waveSetting.bonusPrefabSource)
                                            {
                                                ...
                                                case WaveSpecifics.SpawnOrigin.Specific:
                                                    PoolBossEditorUtility.DisplayPrefab(ref _isDirty, _settings, ref waveSetting.bonusPrefabSpecific, ref waveSetting.bonusPrefabCategoryName,
                                                        ...
                                                        ref waveSetting.shouldBonusPrefabSelectFromPB, ref waveSetting.bonusPrefabFrom, false, "Bonus Prefab"
#if ADDRESSABLES_ENABLED
                                                    , ref waveSetting.bonusPrefabAddressable
                                                    , serializedObject
                                                    , serializedObject.FindProperty(nameof(LevelWave.bonusPrefabAddressable))
#endif
                                                    );
            ...
            if (_settings.showCustomEvents)
            {
                ...
                for (var c = 0; c < _settings.customEventCategories.Count; c++)
                {
                    ...
                    if (!cat.IsEditing || Application.isPlaying)
                    {
                        ...
                        DTInspectorUtility.ShowCollapsibleSectionInline(ref state2, text2);
                        ...
                        if (!Application.isPlaying)
                        {
                            ...
                            if (GUILayout.Button(new GUIContent("Delete", "Click to delete Category"),
                                EditorStyles.toolbarButton, GUILayout.MaxWidth(51)))
                            {
                                catToDelete = cat;
                            }
                        }
        ...
        private void CreateSpawner()
        {
            ...
            var spawnersHolder = CoreMonoHelper.GetChildTransform(_settings.transform, LevelSettings.SpawnerContainerTransName);
        ...
        private void CreatePrefabPool()
        {
            ...
            var poolsHolder = CoreMonoHelper.GetChildTransform(_settings.transform, LevelSettings.PrefabPoolsContainerTransName);
            if (poolsHolder == null)
            {
            ...
            var dupe = CoreMonoHelper.GetChildTransform(poolsHolder, newPrefabPoolName);
            if (dupe != null)
            {
