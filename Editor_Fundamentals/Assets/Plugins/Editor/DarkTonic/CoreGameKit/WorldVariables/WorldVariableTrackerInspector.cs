...
namespace DarkTonic.CoreGameKit.EditorScripts
{
    ...
    public class WorldVariableTrackerInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (DTInspectorUtility.IsPrefabInProjectView(_holder.gameObject))
            {
        ...
        if (_maInScene) {
...
#if MASTERAUDIO_ENABLED
                    CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Variable Changed Sound", aStat, ref _isDirty, ref aStat.variableChangedSound);
#endif
            ...
            if (statToRemove != null)
            {
                ...
                if (PrefabUtility.IsPartOfPrefabInstance(statToRemove)) {
                    var prefabPath = PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(statToRemove);
                    GameObject levelSettings = PrefabUtility.LoadPrefabContents(prefabPath);

                    var tracker = levelSettings.transform.Find(LevelSettings.WorldVariablesContainerTransName);
                    if (tracker != null) {
                        var deadTrans = tracker.Find(statToRemove.name);
                    ...
                    PrefabUtility.SaveAsPrefabAsset(levelSettings, prefabPath);
                    PrefabUtility.UnloadPrefabContents(levelSettings);
                }

                if (!wasDestroyed) {
                    UndoHelper.DestroyForUndo(statToRemove.gameObject);
                }
        ...
        private void CreateNewVariable(string varName, WorldVariableTracker.VariableType varType)
        {
            ...
            var match = CoreMonoHelper.GetChildTransform(_holder.transform, varName);
            if (match != null)
            {
