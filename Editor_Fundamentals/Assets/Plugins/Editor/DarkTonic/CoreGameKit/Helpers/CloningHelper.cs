...
namespace DarkTonic.CoreGameKit.EditorScripts
{
    ...
    public static class CloningHelper
    {
        ...
        public static LevelWave CloneLevelWave(LevelWave levelWave)
        {
            ...
            for (var i = 0; i < levelWave.completionCustomEvents.Count; i++) {
                var anEvent = levelWave.completionCustomEvents[i];
                clone.completionCustomEvents.Add(new CGKCustomEventToFire
                {
                    CustomEventName = anEvent.CustomEventName
                });
            }
        ...
        public static WaveSpecifics CloneWave(WaveSpecifics sourceWave)
        {
            ...
            for (var i = 0; i < sourceWave.waveRepeatCustomEvents.Count; i++)
            {
                var anEvent = sourceWave.waveRepeatCustomEvents[i];
                clone.waveRepeatCustomEvents.Add(new CGKCustomEventToFire
                {
                    CustomEventName = anEvent.CustomEventName
                });
            }
            ...
            KillerVariablesHelper.CloneKillerInt(clone.doNotSpawnIfSpawnCapReached, sourceWave.doNotSpawnIfSpawnCapReached);
