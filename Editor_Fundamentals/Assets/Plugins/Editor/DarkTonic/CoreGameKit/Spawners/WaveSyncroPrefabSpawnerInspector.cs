...
namespace DarkTonic.CoreGameKit.EditorScripts
{
    ...
    public class WaveSyncroPrefabSpawnerInspector : Editor
    {
        ...
        DTInspectorUtility.EndGroupHeader();
            ...
            DTInspectorUtility.ShowCollapsibleSectionInline(ref newExpanded, text);
            ...
            if (_settings.activeMode != LevelSettings.ActiveItemMode.Never)
            {
                ...
                GUI.contentColor = Color.white;
                GUILayout.Space(2);
                DTInspectorUtility.AddHelpIconNoStyle("http://www.dtdevtools.com/docs/coregamekit/SyncroSpawners.htm#WaveSettings", 1);
                    ...
                    for (var w = 0; w < filteredWaves.Count; w++)
                    {
                        ...
                        switch (waveSetting.spawnSource)
                        {
                            case WaveSpecifics.SpawnOrigin.Specific:
                                ...
                                var waveSpecs = serializedObject.FindProperty(nameof(WaveSyncroPrefabSpawner.waveSpecs));
                                for (var j = 0; j < waveSpecs.arraySize; j++)
                                {
                                    var aWave = waveSpecs.GetArrayElementAtIndex(j);
                                    var propLevelNum = aWave.FindPropertyRelative(nameof(WaveSpecifics.SpawnLevelNumber));
                                    var propWaveNum = aWave.FindPropertyRelative(nameof(WaveSpecifics.SpawnWaveNumber));

                                    if (propLevelNum.intValue != waveSetting.SpawnLevelNumber || propWaveNum.intValue != waveSetting.SpawnWaveNumber)
                                    {
                                        continue;
                                    }

                                    serializedProp = aWave.FindPropertyRelative(nameof(WaveSpecifics.prefabToSpawnAddressable));
                                }
                                ...
                                PoolBossEditorUtility.DisplayPrefab(ref _isDirty, _settings, ref waveSetting.prefabToSpawn, ref waveSetting.prefabToSpawnCategoryName,
                                    ...
                                    ref waveSetting.shouldPrefabToSpawnSelectFromPB,
                                    ref waveSetting.spawnFrom,
                                    false,
                                    "Prefab To Spawn"
#if ADDRESSABLES_ENABLED
    ...
    , serializedProp
                        ...
                        if (waveSetting.enableLimits)
                        {
                            ...
                            if (waveSetting.limitUseSpawnCap)
                            {
                                ...
                                if (waveSetting.doNotSpawnIfSpawnCapReached.Value > waveSetting.MinToSpwn.Value)
                                {
                                    DTInspectorUtility.ShowLargeBarAlertBox("Your Spawn Cap is higher than Min To Spawn, so it will never come into affect.");
                                }
                                else if (waveSetting.doNotSpawnIfSpawnCapReached.Value > waveSetting.MaxToSpwn.Value)
                                {
                                ...
                                switch (waveSetting.bonusPrefabSource)
                                {
                                    ...
                                    case WaveSpecifics.SpawnOrigin.Specific:
                                        ...
                                        for (var j = 0; j < waveSpecs.arraySize; j++)
                                        {
                                            ...
                                            serializedProp = aWave.FindPropertyRelative(nameof(WaveSpecifics.bonusPrefabAddressable));
                                        }
