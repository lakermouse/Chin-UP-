    ...
    public class TriggeredSpawnerV2Inspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_levelSettingsInScene)
            {
                ...
                DTInspectorUtility.ShowRedErrorBox("Cannot display Triggered Spawner V2 configuration without LevelSettings Game Object in the Scene.");
                return;
            }
            ...
            if (_settings.gameObject.DestroyChildrenIfAnyInPrefab())
            {
                _isDirty = true;
            }
        ...
        if (_maInScene) {
            ...
            var newExpand = DTInspectorUtility.Foldout(_settings.spawnSettingsExpanded, " Spawn Settings");
            if (newExpand != _settings.spawnSettingsExpanded)
            {
                UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle expand Spawn Settings");
                _settings.spawnSettingsExpanded = newExpand;
            }
            EditorGUILayout.EndVertical();
            EditorGUI.indentLevel = 0;
            ...
            if (_settings.spawnSettingsExpanded)
            {
                var newOutside = EditorGUILayout.Toggle(new GUIContent("Spawn Outside", "If this is checked, everything spawned from this will not reside under this Spawner in the Hierarchy, but instead with no parent Game Object. Note that this can change the size of what's spawned depending on what you have chosen for Spawn Scale Mode."), _settings.spawnOutsidePool);
                if (newOutside != _settings.spawnOutsidePool)
                {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Spawn Outside");
                    _settings.spawnOutsidePool = newOutside;
                }

                if (_settings.spawnOutsidePool)
                {
                    var newFollow = EditorGUILayout.Toggle(new GUIContent("Spawn Follows Spawner", "If this is checked, everything spawned from this will follow the Spawner as long as the Spawner is enabled. It will maintain its initial distance (offset) as the distance at all times."), _settings.spawnsFollowThis);
                    if (newFollow != _settings.spawnsFollowThis)
                    {
                        UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Spawn Follows Spawner");
                        _settings.spawnsFollowThis = newFollow;
                    }

                    var newNoSpawner = (TriggeredSpawnerV2.SpawnerGoneBehavior)EditorGUILayout.EnumPopup("Spawner Disabled Behavior", _settings.spawnerGoneBehavior);
                    if (newNoSpawner != _settings.spawnerGoneBehavior)
                    {
                        UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "Spawner Disabled Behavior");
                        _settings.spawnerGoneBehavior = newNoSpawner;
                    }

                    if (_settings.spawnerGoneBehavior == TriggeredSpawnerV2.SpawnerGoneBehavior.StopAllParticles)
                    {
                        var newDisableChild = EditorGUILayout.Toggle("Disable Child Particles", _settings.spawnerGoneStopChildParticles);
                        if (newDisableChild != _settings.spawnerGoneStopChildParticles)
                        {
                            UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Disable Child Particles");
                            _settings.spawnerGoneStopChildParticles = newDisableChild;
                        }
                    }
                }
        ...
        DTInspectorUtility.StartGroupHeader();
        var newMA = DTInspectorUtility.Foldout(_settings.masterAudioSoundsExpanded, "Master Audio Sounds");
        if (newMA != _settings.masterAudioSoundsExpanded) {
            UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle expand Master Audio Sounds");
            _settings.masterAudioSoundsExpanded = newMA;
        }

        if (_settings.masterAudioSoundsExpanded) {
            DTInspectorUtility.BeginGroupedControls();
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Item Spawned Sound", _settings, ref _isDirty, ref _settings.itemfailedToSpawnSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Item Failed To Spawn Sound", _settings, ref _isDirty, ref _settings.itemSpawnedSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Wave Finished Spawning Sound", _settings, ref _isDirty, ref _settings.waveFinishedSpawningSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Wave Start Sound", _settings, ref _isDirty, ref _settings.waveStartSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Elim. Wave Completed Sound", _settings, ref _isDirty, ref _settings.eliminationWaveCompletedSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Wave Repeat Sound", _settings, ref _isDirty, ref _settings.waveRepeatSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Spawned Despawning Sound", _settings, ref _isDirty, ref _settings.spawnerDespawningSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Spawn Sound", _settings, ref _isDirty, ref _settings.spawningSound);
            CGKMAHelper.SoundGroupField(_maInScene, _groupNames, "Despawn Sound", _settings, ref _isDirty, ref _settings.despawnedSound);
        ...
        DTInspectorUtility.EndGroupHeader();
            ...
            {
                RenderTriggeredWave("enableWaves", 0, ref _settings.enableWaves, GetFirstWaveIfAny(_settings.enableWaves), "Enabled Event", TriggeredSpawner.EventType.OnEnabled);
            }
            if (IsFirstWaveEnabled(_settings.disableWaves))
            {
                RenderTriggeredWave("disableWaves", 0, ref _settings.disableWaves, GetFirstWaveIfAny(_settings.disableWaves), "Disabled Event", TriggeredSpawner.EventType.OnDisabled);
            }
            if (IsFirstWaveEnabled(_settings.visibleWaves))
            {
                RenderTriggeredWave("visibleWaves", 0, ref _settings.visibleWaves, GetFirstWaveIfAny(_settings.visibleWaves), "Visible Event", TriggeredSpawner.EventType.Visible);
            }
            if (IsFirstWaveEnabled(_settings.invisibleWaves))
            {
                RenderTriggeredWave("invisibleWaves", 0, ref _settings.invisibleWaves, GetFirstWaveIfAny(_settings.invisibleWaves), "Invisible Event", TriggeredSpawner.EventType.Invisible);
            }
            if (IsFirstWaveEnabled(_settings.mouseOverWaves))
            {
                RenderTriggeredWave("mouseOverWaves", 0, ref _settings.mouseOverWaves, GetFirstWaveIfAny(_settings.mouseOverWaves), "Mouse Over (Legacy) Event", TriggeredSpawner.EventType.MouseOver_Legacy);
            }
            if (IsFirstWaveEnabled(_settings.mouseClickWaves))
            {
                RenderTriggeredWave("mouseClickWaves", 0, ref _settings.mouseClickWaves, GetFirstWaveIfAny(_settings.mouseClickWaves), "Mouse Click (Legacy) Event", TriggeredSpawner.EventType.MouseClick_Legacy);
            }
            ...
            if (showNewUIEvents)
            {
                ...
                {
                    RenderTriggeredWave("unitySliderChangedWaves", 0, ref _settings.unitySliderChangedWaves, GetFirstWaveIfAny(_settings.unitySliderChangedWaves), "Slider Changed (uGUI) Event", TriggeredSpawner.EventType.SliderChanged_uGUI);
                }
                if (_hasButton && IsFirstWaveEnabled(_settings.unityButtonClickedWaves))
                {
                    RenderTriggeredWave("unityButtonClickedWaves", 0, ref _settings.unityButtonClickedWaves, GetFirstWaveIfAny(_settings.unityButtonClickedWaves), "Button Click (uGUI) Event", TriggeredSpawner.EventType.ButtonClicked_uGUI);
                }
                ...
                if (_hasRect)
                {
                    ...
                    {
                        RenderTriggeredWave("unityPointerDownWaves", 0, ref _settings.unityPointerDownWaves, GetFirstWaveIfAny(_settings.unityPointerDownWaves), "Pointer Down (uGUI) Event", TriggeredSpawner.EventType.PointerDown_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityPointerUpWaves))
                    {
                        RenderTriggeredWave("unityPointerUpWaves", 0, ref _settings.unityPointerUpWaves, GetFirstWaveIfAny(_settings.unityPointerUpWaves), "Pointer Up (uGUI) Event", TriggeredSpawner.EventType.PointerUp_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityPointerEnterWaves))
                    {
                        RenderTriggeredWave("unityPointerEnterWaves", 0, ref _settings.unityPointerEnterWaves, GetFirstWaveIfAny(_settings.unityPointerEnterWaves), "Pointer Enter (uGUI) Event", TriggeredSpawner.EventType.PointerEnter_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityPointerExitWaves))
                    {
                        RenderTriggeredWave("unityPointerExitWaves", 0, ref _settings.unityPointerExitWaves, GetFirstWaveIfAny(_settings.unityPointerExitWaves), "Pointer Exit (uGUI) Event", TriggeredSpawner.EventType.PointerExit_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityDragWaves))
                    {
                        RenderTriggeredWave("unityDragWaves", 0, ref _settings.unityDragWaves, GetFirstWaveIfAny(_settings.unityDragWaves), "Drag (uGUI) Event", TriggeredSpawner.EventType.Drag_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityDropWaves))
                    {
                        RenderTriggeredWave("unityDropWaves", 0, ref _settings.unityDropWaves, GetFirstWaveIfAny(_settings.unityDropWaves), "Drop (uGUI) Event", TriggeredSpawner.EventType.Drop_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityScrollWaves))
                    {
                        RenderTriggeredWave("unityScrollWaves", 0, ref _settings.unityScrollWaves, GetFirstWaveIfAny(_settings.unityScrollWaves), "Scroll (uGUI) Event", TriggeredSpawner.EventType.Scroll_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityUpdateSelectedWaves))
                    {
                        RenderTriggeredWave("unityUpdateSelectedWaves", 0, ref _settings.unityUpdateSelectedWaves, GetFirstWaveIfAny(_settings.unityUpdateSelectedWaves), "Update Selected (uGUI) Event", TriggeredSpawner.EventType.UpdateSelected_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unitySelectWaves))
                    {
                        RenderTriggeredWave("unitySelectWaves", 0, ref _settings.unitySelectWaves, GetFirstWaveIfAny(_settings.unitySelectWaves), "Select (uGUI) Event", TriggeredSpawner.EventType.Select_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityDeselectWaves))
                    {
                        RenderTriggeredWave("unityDeselectWaves", 0, ref _settings.unityDeselectWaves, GetFirstWaveIfAny(_settings.unityDeselectWaves), "Deselect (uGUI) Event", TriggeredSpawner.EventType.Deselect_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityMoveWaves))
                    {
                        RenderTriggeredWave("unityMoveWaves", 0, ref _settings.unityMoveWaves, GetFirstWaveIfAny(_settings.unityMoveWaves), "Move (uGUI) Event", TriggeredSpawner.EventType.Move_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityInitializePotentialDragWaves))
                    {
                        RenderTriggeredWave("unityInitializePotentialDragWaves", 0, ref _settings.unityInitializePotentialDragWaves, GetFirstWaveIfAny(_settings.unityInitializePotentialDragWaves), "Init. Potential Drag (uGUI) Event", TriggeredSpawner.EventType.InitializePotentialDrag_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityBeginDragWaves))
                    {
                        RenderTriggeredWave("unityBeginDragWaves", 0, ref _settings.unityBeginDragWaves, GetFirstWaveIfAny(_settings.unityBeginDragWaves), "Begin Drag (uGUI) Event", TriggeredSpawner.EventType.BeginDrag_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityEndDragWaves))
                    {
                        RenderTriggeredWave("unityEndDragWaves", 0, ref _settings.unityEndDragWaves, GetFirstWaveIfAny(_settings.unityEndDragWaves), "End Drag (uGUI) Event", TriggeredSpawner.EventType.EndDrag_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unitySubmitWaves))
                    {
                        RenderTriggeredWave("unitySubmitWaves", 0, ref _settings.unitySubmitWaves, GetFirstWaveIfAny(_settings.unitySubmitWaves), "Submit (uGUI) Event", TriggeredSpawner.EventType.Submit_uGUI);
                    }
                    if (IsFirstWaveEnabled(_settings.unityCancelWaves))
                    {
                        RenderTriggeredWave("unityCancelWaves", 0, ref _settings.unityCancelWaves, GetFirstWaveIfAny(_settings.unityCancelWaves), "Cancel (uGUI) Event", TriggeredSpawner.EventType.Cancel_uGUI);
                    }
                }
            }
            ...
            {
                RenderTriggeredWave("collisionWaves", 0, ref _settings.collisionWaves, GetFirstWaveIfAny(_settings.collisionWaves), "Collision Enter Event", TriggeredSpawner.EventType.OnCollision);
            }
            if (IsFirstWaveEnabled(_settings.triggerEnterWaves))
            {
                RenderTriggeredWave("triggerEnterWaves", 0, ref _settings.triggerEnterWaves, GetFirstWaveIfAny(_settings.triggerEnterWaves), "Trigger Enter Event", TriggeredSpawner.EventType.OnTriggerEnter);
            }
            if (IsFirstWaveEnabled(_settings.triggerStayWaves))
            {
                RenderTriggeredWave("triggerStayWaves", 0, ref _settings.triggerStayWaves, GetFirstWaveIfAny(_settings.triggerStayWaves), "Trigger Stay Event", TriggeredSpawner.EventType.OnTriggerStay);
            }
            if (IsFirstWaveEnabled(_settings.triggerExitWaves))
            {
                RenderTriggeredWave("triggerExitWaves", 0, ref _settings.triggerExitWaves, GetFirstWaveIfAny(_settings.triggerExitWaves), "Trigger Exit Event", TriggeredSpawner.EventType.OnTriggerExit);
            }

            // Unity 4.3 Events
            if (IsFirstWaveEnabled(_settings.collision2dWaves))
            {
                RenderTriggeredWave("collision2dWaves", 0, ref _settings.collision2dWaves, GetFirstWaveIfAny(_settings.collision2dWaves), "2D Collision Enter Event", TriggeredSpawner.EventType.OnCollision2D);
            }

            if (IsFirstWaveEnabled(_settings.triggerEnter2dWaves))
            {
                RenderTriggeredWave("triggerEnter2dWaves", 0, ref _settings.triggerEnter2dWaves, GetFirstWaveIfAny(_settings.triggerEnter2dWaves), "2D Trigger Enter Event", TriggeredSpawner.EventType.OnTriggerEnter2D);
            }

            if (IsFirstWaveEnabled(_settings.triggerStay2dWaves))
            {
                RenderTriggeredWave("triggerStay2dWaves", 0, ref _settings.triggerStay2dWaves, GetFirstWaveIfAny(_settings.triggerStay2dWaves), "2D Trigger Stay Event", TriggeredSpawner.EventType.OnTriggerStay2D);
            }

            if (IsFirstWaveEnabled(_settings.triggerExit2dWaves))
            {
                RenderTriggeredWave("triggerExit2dWaves", 0, ref _settings.triggerExit2dWaves, GetFirstWaveIfAny(_settings.triggerExit2dWaves), "2D Trigger Exit Event", TriggeredSpawner.EventType.OnTriggerExit2D);
            }

            // code triggered event
            if (IsFirstWaveEnabled(_settings.codeTriggeredWaves1))
            {
                RenderTriggeredWave("codeTriggeredWaves1", 0, ref _settings.codeTriggeredWaves1, GetFirstWaveIfAny(_settings.codeTriggeredWaves1), "Code-Triggered Event 1", TriggeredSpawner.EventType.CodeTriggered1);
            }
            if (IsFirstWaveEnabled(_settings.codeTriggeredWaves2))
            {
                RenderTriggeredWave("codeTriggeredWaves2", 0, ref _settings.codeTriggeredWaves2, GetFirstWaveIfAny(_settings.codeTriggeredWaves2), "Code-Triggered Event 2", TriggeredSpawner.EventType.CodeTriggered2);
            }

            // Pool Boss & Pool Manager events (same for both).
            if (IsFirstWaveEnabled(_settings.spawnedWaves))
            {
                RenderTriggeredWave("spawnedWaves", 0, ref _settings.spawnedWaves, GetFirstWaveIfAny(_settings.spawnedWaves), "Spawned Event", TriggeredSpawner.EventType.OnSpawned);
            }
            if (IsFirstWaveEnabled(_settings.despawnedWaves))
            {
                RenderTriggeredWave("despawnedWaves", 0, ref _settings.despawnedWaves, GetFirstWaveIfAny(_settings.despawnedWaves), "Despawned Event", TriggeredSpawner.EventType.OnDespawned);
            }

            // NGUI events
            if (IsFirstWaveEnabled(_settings.clickWaves))
            {
                RenderTriggeredWave("clickWaves", 0, ref _settings.clickWaves, GetFirstWaveIfAny(_settings.clickWaves), "NGUI OnClick Event", TriggeredSpawner.EventType.OnClick_NGUI);
            }

            for (var i = 0; i < _settings.userDefinedEventWaves.Count; i++)
            {
                var aWave = _settings.userDefinedEventWaves[i];
                RenderTriggeredWave("userDefinedEventWaves", i, ref _settings.userDefinedEventWaves, aWave, "Custom Event", TriggeredSpawner.EventType.CustomEvent, i);
            }

            bool canVisualize = !Application.isPlaying && !DTInspectorUtility.IsPrefabInProjectView(_settings.gameObject);

            canVisualize = canVisualize && DTInspectorUtility.IsPrefab(_settings.gameObject);
        ...
        private void RenderTriggeredWave(string wavePropertyName, int waveIndex, ref List<TriggeredWaveSpecifics> waveList, TriggeredWaveSpecifics waveSetting, string toggleText, TriggeredSpawner.EventType eventType, int? itemIndex = null)
        {
            ...
            DTInspectorUtility.FunctionButtons customEventButtonPressed = DTInspectorUtility.FunctionButtons.None;
            DTInspectorUtility.FunctionButtons otherButtonPressed = DTInspectorUtility.FunctionButtons.None;
            ...
            if (eventType == TriggeredSpawner.EventType.CustomEvent)
            {
                ...
                customEventButtonPressed = DTInspectorUtility.AddCustomEventIcons(false, false, false, true);
                DTInspectorUtility.AddHelpIconNoStyle("http://www.dtdevtools.com/docs/coregamekit/TriggeredSpawners.htm#WaveSettings", 1);

                switch (customEventButtonPressed)
                {
            ...
            else
            {
                ...
                otherButtonPressed = DTInspectorUtility.AddCustomEventIcons(false, false, false, true);

                switch (otherButtonPressed)
                {
                    case DTInspectorUtility.FunctionButtons.Fire:
            ...
            if (waveSetting.visualizeWave)
            {
                if (!DTInspectorUtility.IsPrefab(_settings.gameObject)) {
                    DTInspectorUtility.ShowColorWarningBox("You cannot Visualize Wave unless you have selected it in Scene View and it is a prefab.");
                }
            }
            ...
            switch (waveSetting.spawnSource)
            {
                ...
                var waveArrayProp = serializedObject.FindProperty(wavePropertyName);
                if (waveArrayProp.arraySize == 0)
                {
                    break;
                }
                var waveProp = waveArrayProp.GetArrayElementAtIndex(waveIndex);
                var serializedProp = waveProp.FindPropertyRelative(nameof(TriggeredWaveSpecifics.prefabToSpawnAddressable));
#endif
                    PoolBossEditorUtility.DisplayPrefab(ref _isDirty, _settings, ref waveSetting.prefabToSpawn, ref waveSetting.prefabToSpawnCategoryName,
                        ref waveSetting.prefabToSpawnPoolItemName,
                        ref waveSetting.shouldPrefabToSpawnSelectFromPB, ref waveSetting.spawnFrom, false, "Prefab To Spawn"
#if ADDRESSABLES_ENABLED
                , ref waveSetting.prefabToSpawnAddressable
                , serializedObject
                , serializedProp
#endif
);
                    ...
                    case PoolBoss.PrefabSource.Addressable:
                        if (!CGKAddressableEditorHelper.IsAddressableValid(waveSetting.prefabToSpawnAddressable))
                        {
                            DTInspectorUtility.ShowRedErrorBox("Please specify a prefab to spawn.");
                        }
                        break;
                ...
                if (waveSetting.useSpawnBonusPrefab)
                {
                    ...
                    switch (waveSetting.bonusPrefabSource)
                    {
                        ...
                        case WaveSpecifics.SpawnOrigin.Specific:
                            ...
                            var waveProp = waveArrayProp.GetArrayElementAtIndex(waveIndex);
                            var bonusPrefabProperty = waveProp.FindPropertyRelative(nameof(TriggeredWaveSpecifics.bonusPrefabAddressable));
#endif
                            PoolBossEditorUtility.DisplayPrefab(ref _isDirty, _settings, ref waveSetting.bonusPrefabSpecific, ref waveSetting.bonusPrefabCategoryName,
                                ref waveSetting.bonusPrefabPoolItemName,
                                ref waveSetting.shouldPrefabToSpawnSelectFromPB, ref waveSetting.bonusPrefabFrom, false, "Bonus Prefab"
#if ADDRESSABLES_ENABLED
                            , ref waveSetting.bonusPrefabAddressable
                            , serializedObject
                            , bonusPrefabProperty
#endif
                            );
            ...
            DTInspectorUtility.StartGroupHeader(1);
            var newUseLimit = EditorGUILayout.BeginToggleGroup(" Spawn Limit Controls", waveSetting.enableLimits);
            if (newUseLimit != waveSetting.enableLimits)
            {
                UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Spawn Limit Controls");
                waveSetting.enableLimits = newUseLimit;
            }
            DTInspectorUtility.EndGroupHeader();
            if (waveSetting.enableLimits)
            {
                DTInspectorUtility.ShowColorWarningBox("Stop spawning until all spawns from wave satisfy the following");

                DTInspectorUtility.BeginGroupedControls();
                var newUseMinDist = EditorGUILayout.BeginToggleGroup(" Use Min Distance", waveSetting.limitUseMinDistance);
                if (newUseMinDist != waveSetting.limitUseMinDistance)
                {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Use Use Min Distance");
                    waveSetting.limitUseMinDistance = newUseMinDist;
                }

                if (waveSetting.limitUseMinDistance)
                {
                ...
                DTInspectorUtility.BeginGroupedControls();
                var newUseCap = EditorGUILayout.BeginToggleGroup("Use Spawn Cap", waveSetting.limitUseSpawnCap);
                if (newUseCap != waveSetting.limitUseSpawnCap)
                {
                    UndoHelper.RecordObjectPropertyForUndo(ref _isDirty, _settings, "toggle Use Spawn Cap");
                    waveSetting.limitUseSpawnCap = newUseCap;
                }

                if (waveSetting.limitUseSpawnCap)
                {
                    KillerVariablesHelper.DisplayKillerInt(ref _isDirty, waveSetting.doNotSpawnIfSpawnCapReached, "Spawn Cap", _settings);
                    if (waveSetting.doNotSpawnIfSpawnCapReached.Value > waveSetting.NumberToSpwn.Value)
                    {
                        DTInspectorUtility.ShowLargeBarAlertBox("Your Spawn Cap is higher than Min To Spawn, so it will never come into affect.");
                    }
                    else if (waveSetting.doNotSpawnIfSpawnCapReached.Value > waveSetting.MaxToSpawn.Value)
                    {
                        DTInspectorUtility.ShowLargeBarAlertBox("Your Spawn Cap is higher than Max To Spawn, so it may never come into affect.");
                    }
                }
                EditorGUILayout.EndToggleGroup();
                DTInspectorUtility.EndGroupedControls();
