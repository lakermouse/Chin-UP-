        ...
        public override void OnInspectorGUI()
        {
            ...
            if (_settings.isExpanded)
            {
                ...
                for (var i = 0; i < _settings.poolItems.Count; i++)
                {
                    ...
                    PoolBossEditorUtility.DisplayPrefab(ref _isDirty, _settings, ref item.prefabToSpawn, ref item.prefabPoolBossCategory,
                        ref item.prefabPoolBossPoolItemName,
                        ref item.shouldSelectFromPB, ref item.prefabFrom, false, "Prefab Pool Item"
#if ADDRESSABLES_ENABLED
    , ref item.prefabAddressable
    , serializedObject
    , serializedObject.FindProperty(nameof(WavePrefabPoolItem.prefabAddressable))
#endif
                    );
