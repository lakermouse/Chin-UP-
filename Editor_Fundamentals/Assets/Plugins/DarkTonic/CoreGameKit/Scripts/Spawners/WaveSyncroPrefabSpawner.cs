...
namespace DarkTonic.CoreGameKit {
    ...
    public class WaveSyncroPrefabSpawner : MonoBehaviour {
        ...
        private void Update() {
            ...
            var pauseValue = Time.time - _waveStartTime - LevelSettings.WavePauseTime;
            if (pauseValue < 0) {
                pauseValue = 0; // for wave repeats in this spawner that may result in a negative value and erroneously delayed wave repeats.
            }

            if (_waveFinishedSpawning // wave is finished and in wait mode 
                || (pauseValue < WaveDelay) // shouldn't spawn yet because of Wave Delay setting. 
...
#if MASTERAUDIO_ENABLED
                    CGKMARuntimeHelper.PlaySoundFollowTransform(waveFinishedSpawningSound, transform);
#endif
                    if (listener != null) {
        ...
        public void RemoveSpawnedMember(Transform transMember) {
            ...
            if (_spawnedWaveMembers.Contains(transMember)) {
                ...
                if (_itemsToCompleteWave <= 0 || _spawnedWaveMembers.Count <= 0) {
        ...
        /*! \cond PRIVATE */
        public bool WaveChange(bool isRestart, out SyncroSpawnerWaveSpecs runtimeWaveSpecs) {
            _lastPrefabKilled = null;
            runtimeWaveSpecs = null;
...
#if MASTERAUDIO_ENABLED
            CGKMARuntimeHelper.PlaySoundFollowTransform(waveStartSound, transform);
#endif
            if (listener != null) {
                listener.WaveStart(_currentWave);
            }

            runtimeWaveSpecs = new SyncroSpawnerWaveSpecs {
                NumberToSpawn = _currentWaveSize
            };
            return true;
        }
...
#if MASTERAUDIO_ENABLED
            CGKMARuntimeHelper.PlaySoundFollowTransform(waveRepeatSound, transform);
#endif
            if (listener != null) {
...
#if MASTERAUDIO_ENABLED
            CGKMARuntimeHelper.PlaySoundFollowTransform(itemSpawnedSound, transform);
#endif
            if (listener != null) {
        ...
        protected virtual bool CanSpawnOne() {
            ...
            if (_currentWave.limitUseSpawnCap) {
                if (_spawnedWaveMembers.Count >= _currentWave.doNotSpawnIfSpawnCapReached.Value) {
                    return false;
                }
            }

            if (_currentWave.limitUseMinDistance) {
        ...
        protected virtual Transform GetSpawnable(WaveSpecifics wave) {
            ...
            switch (wave.spawnFrom)
            {
                ...
                case PoolBoss.PrefabSource.Addressable:
                    var go = PoolAddressableOptimizer.RetrieveStoredHandle(wave.prefabToSpawnAddressable);
                    if (go == null)
                    {
                        return null;
                    }
                    ...
                    switch (waveBeat.bonusPrefabSource) {
                        case WaveSpecifics.SpawnOrigin.PrefabPool:
                            ...
                            for (var i = 0; i < numToSpawn; i++) {
                                ...
                                var spawned = SpawnWaveItem(prefabToSpawn, spawnPosition, Quaternion.identity);
                                if (spawned == null) {
                                    break;
                                }
                            }
                        ...
                        case WaveSpecifics.SpawnOrigin.Specific:
                            ...
                            for (var i = 0; i < numberToSpawn; i++) {
                                var spawned = SpawnWaveItem(waveBeat.bonusPrefabSpecific, spawnPosition, Quaternion.identity);
                                if (spawned == null) {
                                    break;
                                }
                            }
...
#if ADDRESSABLES_ENABLED
                case PoolBoss.PrefabSource.Addressable:
                    var go = PoolAddressableOptimizer.RetrieveStoredHandle(waveBeat.bonusPrefabAddressable);
                    ...
                    {
                        var spawned = SpawnWaveItem(go.transform, spawnPosition, Quaternion.identity);
                        if (spawned == null)
                        {
                            break;
                        }
        ...
        private void CheckForValidVariablesForWave(WaveSpecifics wave) {
            ...
            if (wave.enableLimits) {
                ...
                if (wave.limitUseSpawnCap) {
                    wave.doNotSpawnIfSpawnCapReached.LogIfInvalid(_trans, "Spawn Cap", wave.SpawnLevelNumber, wave.SpawnWaveNumber);
                }
            }
        ...
        private bool SetupNextWave(bool scanForWave, bool isRestart) {
            ...
            _itemsToCompleteWave = (int)Math.Round(_currentWaveSize * _currentWave.waveCompletePercentage * .01f);
            ...
            if (_currentWave.enableLimits && _currentWave.limitUseMinDistance) {
        ...
        private Transform SpawnOne(bool fromExternalScript = false) {
            ...
            var hasMissingPrefab = false;
            switch (_currentWave.spawnFrom)
            {
                case PoolBoss.PrefabSource.Prefab:
                    if (prefabToSpawn == null)
                    {
...
#if ADDRESSABLES_ENABLED
                case PoolBoss.PrefabSource.Addressable:
                    if (!PoolAddressableOptimizer.IsAddressableValid(_currentWave.prefabToSpawnAddressable))
                    {
                        hasMissingPrefab = true;
                    }
                    break;
...
#if MASTERAUDIO_ENABLED
                CGKMARuntimeHelper.PlaySoundFollowTransform(itemfailedToSpawnSound, transform);
#endif
                if (listener != null) {
...
#if MASTERAUDIO_ENABLED
                CGKMARuntimeHelper.PlaySoundFollowTransform(eliminationWaveCompletedSound, transform);
#endif
                if (listener != null) {
