//#define PHOTON_NETWORK // uncomment to use experimental Photon features.
//#define NORMCORE_NETWORK // uncomment to use experimental Normcore.io features.
...
namespace DarkTonic.CoreGameKit {
	...
	Photon.MonoBehaviour {
        ...
        public bool fireCompletedCustomEvent;
        public List<CGKCustomEventToFire> completedCustomEvents = new List<CGKCustomEventToFire>();
	...
	private static PhotonView _view;
...
#else
    #if NORMCORE_NETWORK
            if (prefabTrans.GetComponent<RealtimeView>()) {
                var prefab = prefabTrans.gameObject;
                var prefabObj = Realtime.Instantiate(prefab.name, position: new Vector3(0f, 0f, 0f), rotation: Quaternion.identity, ownedByClient: true, preventOwnershipTakeover: true, destroyWhenOwnerOrLastClientLeaves: true);
                createdObjTransform = prefabObj.transform;
            } else {
                createdObjTransform = Instantiate(prefabTrans, Trans.position, prefabTrans.rotation) as Transform;
            }
    #else
        ...
        private void CheckIfInitFinished()
        {
            ...
            if (fireCompletedCustomEvent)
            {
                for (var i = 0; i < completedCustomEvents.Count; i++)
                {
                    var anEvent = completedCustomEvents[i];
                    LevelSettings.FireCustomEventIfValid(anEvent.CustomEventName, transform);
                }
            }
        ...
        /// <param name="prefabAddressable">Prefab Addressable, if Addressable feature enabled</param>
        ...
        private void FillItemPool(PoolBossItem item, Transform transformTemplate, Action itemCreatedCallback)
        {
            ...
            {
                LevelSettings.LogIfNew("You have more than one instance of '" + itemName + "' in Pool Boss. Skipping creating pool for the second instance.");
                CheckIfInitFinished();
                return;
            }
        ...
        public static Transform Spawn(string itemName, Vector3 position, Quaternion rotation, Transform parentTransform) {
            if (!PoolItemsByName.ContainsKey(itemName)) {
                LevelSettings.LogIfNew("The Transform '" + itemName + "' has no pool in Pool Boss. It either hasn't been configured or is still preparing the pool during startup.");
                return null;
            }
        ...
        private static void EnableNavMeshAgentForObject(Transform navMeshTransform) {
            if (!CoreMonoHelper.IsActive(navMeshTransform.gameObject)) {
                return;
            }
		...
		/// <param name="addressable">Pass in the the AssetReference of the Pool Item you are destroying if it is Addressable</param>
