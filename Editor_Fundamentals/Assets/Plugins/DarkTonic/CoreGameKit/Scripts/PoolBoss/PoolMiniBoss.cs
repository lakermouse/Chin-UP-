...
namespace DarkTonic.CoreGameKit
{
    ...
    public class PoolMiniBoss : MonoBehaviour
    {
        ...
        public RemoveBehavior removeBehaviorType = RemoveBehavior.Despawn;
        ...
        public bool fireCompletedCustomEvent = false;
        ...
        public string itemsCreatedSound = MasterAudio.MasterAudio.NoGroupName;
        public string itemsRemovedSound = MasterAudio.MasterAudio.NoGroupName;
#endif
...
#if MASTERAUDIO_ENABLED
            CGKMARuntimeHelper.PlaySoundFollowTransform(itemsCreatedSound, transform);
#endif
        ...
        public void RemoveItems()
        {
            ...
            for (var i = 0; i < poolItems.Count; i++)
            {
                ...
                {
                    // despawn items
                    switch (removeBehaviorType)
                    {
                        case RemoveBehavior.Despawn:
                            PoolBoss.DespawnAllOfPrefab(itemTransform);
                            break;
                        case RemoveBehavior.Kill:
                            PoolBoss.KillAllOfPrefab(itemTransform);
                            break;
                    }
            ...
            for (var i = 0; i < _categories.Count; i++)
            {
                ...
                var matchingCat = PoolBoss.Instance._categories.Find(delegate (PoolBossCategory cat)
                {
                    return cat.CatName == aCat.CatName && cat.CatIsTemporary == true;
                });
                if (matchingCat == null)
                {
                    continue;
...
#if MASTERAUDIO_ENABLED
            CGKMARuntimeHelper.PlaySoundFollowTransform(itemsRemovedSound, transform);
#endif
