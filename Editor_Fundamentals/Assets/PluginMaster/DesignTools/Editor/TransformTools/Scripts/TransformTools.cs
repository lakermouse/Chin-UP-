...
namespace PluginMaster
{
    public static class TransformTools
    {
        ...
        private static Tuple<GameObject, Bounds> GetSelectionBounds(GameObject[] selection, RelativeTo relativeTo,
            Axis axis, bool recursive = true, BoundsUtils.ObjectProperty property = BoundsUtils.ObjectProperty.BOUNDING_BOX)
        {
            ...
            if (anchor != null) return new Tuple<GameObject, Bounds>
                    (anchor, BoundsUtils.GetBoundsRecursive(anchor.transform, recursive, property));
        ...
        public static void Align(GameObject[] selection, RelativeTo relativeTo, Axis axis, Bound bound, bool AlignToAnchor,
            bool filterByTopLevel = true, BoundsUtils.ObjectProperty property = BoundsUtils.ObjectProperty.BOUNDING_BOX,
        ...
        private static Dictionary<(int i, int j, int k), GameObject> SortBySelectionOrder(GameObject[] selection,
            ArrangeData data)
        {
            int i = 0;
            int j = 0;
        ...
        private static Dictionary<(int i, int j, int k), GameObject> SortByPosition(GameObject[] selection,
            ArrangeData data, Bounds selectionBounds)
        {
            ...
            {
                var cellObjectsDict = new Dictionary<(int i, int j, int k),
                    List<(GameObject obj, float sqrDistanceToCorner, float sqrDistanceToCenter)>>();
                foreach (var obj in unsorted)
                {
                    ...
                    else
                    {
                        cellObjectsDict.Add(closestCell.Key,
                            new List<(GameObject obj, float sqrDistanceToCorner, float sqrDistanceToCenter)>());
                        cellObjectsDict[closestCell.Key].Add((obj, minSqrDistanceToCorner, minSqrDistanceToCenter));
                    }
                }
        ...
        private static void GetOverlapedDataAxis(Axis axis, UnoverlapData unoverlapData,
            ...
            UnoverlapData unoverlapData, bool getSolutionVolumen)
        {
            ...
            if (retVal.volume > 0)
            {
                ...
                GetOverlapedDataAxis(Axis.X, unoverlapData, selection, index, b1, b2, getSolutionVolumen,
                    ref retVal, ref minVol, ref bestMove);
                GetOverlapedDataAxis(Axis.Y, unoverlapData, selection, index, b1, b2, getSolutionVolumen,
                    ref retVal, ref minVol, ref bestMove);
                GetOverlapedDataAxis(Axis.Z, unoverlapData, selection, index, b1, b2, getSolutionVolumen,
                    ref retVal, ref minVol, ref bestMove);
            }
            return retVal;
        }

        private static List<OverlapData> GetOverlapedData(OverlapedObject[] selection,
        ...
        private static void EditColliders(Transform target, Transform pivot, string originalMeshPath)
        {
            ...
            {
                colliderChild.transform.SetParent(target);
        ...
        public static void CenterPivot(SkinnedMeshRenderer renderer,
            string savePath, GameObject pivot, List<Transform> otherObjects)
        {
            var target = renderer.transform;
            if (target == null || target.gameObject.scene.rootCount == 0) return;
        ...
        {
            var meshFilters = Array.FindAll(target.GetComponentsInChildren<MeshFilter>(),
            ...
            PlaceOnSurfaceUtils.PlaceOnSurfaceData data, GameObject[] filters)
        {
            ...
            bool Raycast(Vector3 rayOrigin, out RaycastHit hitInfo)
            {
                ...
                bool meshHit = RaycastMesh.Raycast(rayOrigin, worldProjDir,
